...
class Wallee_FeeHelper
{
    public static function removeFeeSurchargeProductsFromCart(Cart $cart)
    {
        $surchargeProductId = Configuration::get(Wallee::CK_SURCHARGE_ITEM);
        ...
        Wallee_VersionAdapter::clearCartRuleStaticCache();
    }
    ...
    public static function addFeeProductToCart(Wallee_Model_MethodConfiguration $methodConfiguration, Cart $cart)
    {
        $feeProductId = Configuration::get(Wallee::CK_FEE_ITEM);
        $feeProduct = new Product($feeProductId, false, Configuration::get('PS_LANG_DEFAULT'), $cart->id_shop);
    ...
    public static function getSurchargeValues(Cart $cart)
    {
        ...
        $amount = (float) Configuration::get(Wallee::CK_SURCHARGE_AMOUNT);
        ...
        $surchargeBaseType = Configuration::get(Wallee::CK_SURCHARGE_BASE);
        ...
        $surchargeBase = (float) Configuration::get(Wallee::CK_SURCHARGE_TOTAL);
        ...
        if ($taxGroup != 0) {
            $addressFactory = Wallee_VersionAdapter::getAddressFactory();
            ...
            if ((int) Configuration::get(Wallee::CK_SURCHARGE_TAX)) {
    ...
    public static function getFeeValues(Cart $cart, Wallee_Model_MethodConfiguration $methodConfiguration)
    {
        $feeProductId = Configuration::get(Wallee::CK_FEE_ITEM);
        $feeProduct = new Product($feeProductId, false, Configuration::get('PS_LANG_DEFAULT'), $cart->id_shop);
        if (!Validate::isLoadedObject($feeProduct)) {

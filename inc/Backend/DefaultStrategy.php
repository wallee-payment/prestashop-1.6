...
class Wallee_Backend_DefaultStrategy implements Wallee_Backend_IStrategy
{
    ...
    private function applyRefundPartialRefundType(Order $order, array $parsedData)
    {
        ...
        if ($parsedData['generateDiscountRefund']) {
            ...
            $cart_rule->description = sprintf(
                Wallee_Helper::getModuleInstance()->l('Credit slip for order #%d', 'defaultstrategy'),
                $order->id
            );
    ...
    public function processCancel(Order $order, array $postData)
    {
        ...
        try {
            ...
            throw new Exception(
                sprintf(
                    Wallee_Helper::getModuleInstance()->l('Could not update the line items at %s. Reason: %s', 'defaultstrategy'),
                    'wallee',

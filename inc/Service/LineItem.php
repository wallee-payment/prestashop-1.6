...
class Wallee_Service_LineItem extends Wallee_Service_Abstract
{
    ...
    public function getItemsFromCart(Cart $cart)
    {
        ...
        foreach ($summary['products'] as $productItem) {
            ...
            $name = empty($productItem['name']) ? Wallee_Helper::getModuleInstance()->l('Product', 'lineitem').' '.$productItem['id_product'] . '-' .
            ...
            if ($productItem['id_product'] == Configuration::get(Wallee::CK_FEE_ITEM) || $productItem['id_product'] == Configuration::get(Wallee::CK_SURCHARGE_ITEM)) {
        ...
        if (count($summary['discounts']) > 0) {
            ...
            foreach ($summary['discounts'] as $discount) {
                $nameBase = empty($discount['description']) ? Wallee_Helper::getModuleInstance()->l('Discount', 'lineitem') : $discount['description'];
        ...
        if ($discountOnly && $shippingItem != null) {
            ...
            $name = empty($discountOnly['name']) ? Wallee_Helper::getModuleInstance()->l('Shipping Discount', 'lineitem') : $discountOnly['name'];
    ...
    protected function getItemsFromOrdersInner(array $orders)
    {
        ...
        foreach ($orders as $order) {
            ...
            foreach ($order->getProducts() as $orderItem) {
                ...
                $name = empty($orderItem['product_name']) ? Wallee_Helper::getModuleInstance()->l('Product', 'lineitem').' '.$orderItem['product_id'] . '-' .
                ...
                if ($orderItem['product_id'] == Configuration::get(Wallee::CK_FEE_ITEM) || $orderItem['product_id'] == Configuration::get(Wallee::CK_SURCHARGE_ITEM)) {
                    $item->setType(\Wallee\Sdk\Model\LineItemType::FEE);
                    $item->setShippingRequired(false);
                }
            ...
            foreach ($order->getCartRules() as $orderCartRule) {
                ...
                $nameBase = empty($orderCartRule['name']) ? Wallee_Helper::getModuleInstance()->l('Discount', 'lineitem') : $orderCartRule['name'];

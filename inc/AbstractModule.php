 ...
 *
 */
abstract class Wallee_AbstractModule extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        if (isset($this->context->cart) && Validate::isLoadedObject($this->context->cart)) {
            Wallee_FeeHelper::removeFeeSurchargeProductsFromCart($this->context->cart);
        }
    ...
    protected function displayForm()
    {
        ...
        $fieldsForm[]['form'] = array(
            'legend' => array(
                'title' => 'wallee ' . $this->l('General Settings', 'abstractmodule')
            ),
            'input' => $generalInputs,
            'buttons' => $buttons
        );
    ...
    protected function getParametersFromMethodConfiguration(
        ...
        $translatedName = Wallee_Helper::translate(
        ...
        if (! empty($methodConfiguration->getImage()) && $methodConfiguration->isShowImage()) {
            $parameters['image'] = Wallee_Helper::getResourceUrl(
                ...
                Wallee_Helper::convertLanguageIdToIETF($cart->id_lang),
                $spaceId,
                $spaceViewId
            );
        }
        $parameters['description'] = '';
        $description = Wallee_Helper::translate(
        ...
        $surchargeValues = Wallee_FeeHelper::getSurchargeValues($cart);
    ...
    public function validateOrder(
        ...
        if ($this->active) {
            ...
            try {
                ...
                $translatedTitel = Wallee_Helper::translate(
                    $methodConfiguration->getTitle(),
                    $cart->id_lang
                );
            ...
            try {
                $transaction = Wallee_Service_Transaction::instance()->confirmTransaction(
                    $dataOrder,
                    $orders
                );
                Wallee_Service_Transaction::instance()->updateTransactionInfo($transaction, $dataOrder);
                $GLOBALS['walleeTransactionIds'] = array('spaceId' => $transaction->getLinkedSpaceId(),'transactionId' => $transaction->getId());
            } catch (Exception $e) {
                ...
                throw new Exception(
                    Wallee_Helper::getModuleInstance()->l('There was a techincal issue, please try again.', 'abstractmodule')
                );
            }
        } else {
            throw new Exception(
    ...
    public function hookDisplayOrderDetail($params)
    {
        ...
        if (in_array(
            ...
            $documentVars['walleeInvoice'] = $this->context->link->getModuleLink(
                'wallee',
                'documents',
                array(
                    'type' => 'invoice',
    ...
    public function hookDisplayBackOfficeHeader($params)
    {
        if (Module::isEnabled($this->name)) {
            try {
                ...
                $this->displayError(
                    $this->l(sprintf('Error migrating the database for %s. Please check the log to resolve the issue.', 'abstractmodule'), 'wallee')
                );
            ...
            if ($this->hasBackendControllerDeleteAccess($backendController)) {
                ...
                if ($strategy->isCancelRequest($order, $cancelParameters)) {
                    ...
                    try {
                        ...
                        Wallee_Service_Refund::instance()->executeRefund(
                            $order,
                            $parsedData
                        );
                        Tools::redirectAdmin(
    ...
    public function hookDisplayAdminOrderLeft($params)
    {
        ...
        $tplVars = array(
            ...
            'methodImage' => Wallee_Helper::getResourceUrl(
                $transactionInfo->getImageBase(),
                $transactionInfo->getImage(),
                Wallee_Helper::convertLanguageIdToIETF($order->id_lang),
                $spaceId,
            ...
            'labelsByGroup' => Wallee_Helper::getGroupedChargeAttemptLabels(
                $transactionInfo
            ),
            'voids' => Wallee_Model_VoidJob::loadByTransactionId($spaceId, $transactionId),
            'completions' => Wallee_Model_CompletionJob::loadByTransactionId(
                $spaceId,
                $transactionId
            ),
            'refunds' => Wallee_Model_RefundJob::loadByTransactionId(
                $spaceId,
                $transactionId
            )
        );
    ...
    public function hookDisplayAdminOrder($params)
    {
        ...
        if ($transactionInfo->getState() == \Wallee\Sdk\Model\TransactionState::AUTHORIZED) {
            ...
            ) && ! Wallee_Model_VoidJob::isVoidRunningForTransaction(
                $transactionInfo->getSpaceId(),
                $transactionInfo->getTransactionId()
            )) {
        ...
        if (Wallee_Model_VoidJob::isVoidRunningForTransaction(
            $transactionInfo->getSpaceId(),
            $transactionInfo->getTransactionId()
        )) {
            $templateVars['voidPending'] = true;
        }
        if (Wallee_Model_RefundJob::isRefundRunningForTransaction(
            $transactionInfo->getSpaceId(),
            $transactionInfo->getTransactionId()
        )) {

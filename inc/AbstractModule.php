...
abstract class Wallee_AbstractModule extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $this->description = sprintf(
            $this->l('This PrestaShop module enables to process payments with %s.','abstractmodule'), 'wallee');
        $this->confirmUninstall = sprintf(
            $this->l('Are you sure you want to uninstall the %s module?','abstractmodule'), 'wallee');
        
        // Remove Fee Item
        if (isset($this->context->cart) && Validate::isLoadedObject($this->context->cart)) {
            Wallee_FeeHelper::removeFeeProductFromCart($this->context->cart);
        }
    ...
    protected function getParametersFromMethodConfiguration(
        Wallee_Model_MethodConfiguration $methodConfiguration, Cart $cart, $shopId,
        $language)
    {
        ...
        $parameters['link'] = $this->context->link->getModuleLink('wallee',
            'payment',
            array(
                'methodId' => $methodConfiguration->getId()
            ), true);
        $name = $methodConfiguration->getConfigurationName();
        $translatedName = Wallee_Helper::translate($methodConfiguration->getTitle(),
            $language);
            ...
            if ($this->hasBackendControllerDeleteAccess($backendController)) {
                ...
                if ($strategy->isCancelRequest($order, $cancelParameters)) {
                    ...
                    try {
                        ...
                        Wallee_Service_Refund::instance()->executeRefund($order,
                            $parsedData);
                        Tools::redirectAdmin(
                            $backendController::$currentIndex . '&id_order=' . $order->id .
    ...
    public function hookDisplayAdminOrderLeft($params)
    {
        ...
        $tplVars = array(
            ...
            'completions' => Wallee_Model_CompletionJob::loadByTransactionId($spaceId,
                $transactionId),
            'refunds' => Wallee_Model_RefundJob::loadByTransactionId($spaceId,
                $transactionId)
        );
        ...
        if ($transactionInfo->getState() == \Wallee\Sdk\Model\TransactionState::AUTHORIZED) {
            if (! Wallee_Model_CompletionJob::isCompletionRunningForTransaction(
                ...
                $templateVars['voidUrl'] = $this->context->link->getAdminLink(
                    'AdminWalleeOrder', true) . '&action=voidOrder&ajax=1&id_order=' .
                     $orderId;
                $templateVars['completionUrl'] = $this->context->link->getAdminLink(
                    'AdminWalleeOrder', true) . '&action=completeOrder&ajax=1&id_order=' .
                     $orderId;
            }
        }
        if (in_array($transactionInfo->getState(),
